<template>
  <div class="box">
    <div class="title">
      ...
      <h2>购物车</h2>
      <p @click="selectchange()">{{isdel?'编辑全部':'完成'}}</p>
    </div>
    <div class="content">
      <div class="trade">
      ...
      <ul>
        <li v-for="data in dataList" :key="data.cartId">
          ...
          <img :src="data.goodsimg" alt="商品图片" @click="godetail(data.goodsId)">
          <div class="goodsitem">
            <div class="goodstitle" @click="godetail(data.goodsId)">{{data.goodsname}}</div>
      ...
      del () {
        for (var i in this.selectList) {
          Axios.post('cart/delete', {
            cartId: this.selectList[i].cartId,
            userId: JSON.parse(localStorage.getItem('token')).id
          }).then(res => {
            if (res.data.code === 1) {
